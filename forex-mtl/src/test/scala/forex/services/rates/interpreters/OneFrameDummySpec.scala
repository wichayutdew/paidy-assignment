...
class OneFrameDummySpec extends AnyWordSpec with Matchers {
  "OneFrameDummy" when {
    "get" should {
      "return a dummy rate for any pair" in new Fixture {
        val pair: Rate.Pair                        = Rate.Pair(CAD, CHF)
        val result: Id[Either[errors.Error, Rate]] = client.get(pair)

        result.isRight shouldBe true
        result.toOption.get.pair.from shouldBe CAD
        result.toOption.get.pair.to shouldBe CHF
