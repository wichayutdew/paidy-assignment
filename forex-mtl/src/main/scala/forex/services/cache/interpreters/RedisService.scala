...
class RedisService[F[_]: Sync](client: RedisCommands[String, String])(implicit meter: Meter)
    extends Algebra[F]
    with AppLogger
  ...
  private val timer                            = TimerMetric("client.time", "RedisService")
  ...
  override def get(key: String): F[Option[String]] =
    measure(timer, Map(MetricsTag.OPERATION -> "get")) {
      Try(client.get(key)).toEither match {
